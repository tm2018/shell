...
install(){
	...
	git clone https://github.com/tm2018/shadowsocksr-python.git shadowsocksr
	...
	cd shadowsocks && python server.py -d start
	status
}

status(){
    if [[ -e /run/shadowsocksr.pid ]]; then
...
uninstall(){
    cd /home/shadowsocksr/shadowsocks && python server.py -d stop
